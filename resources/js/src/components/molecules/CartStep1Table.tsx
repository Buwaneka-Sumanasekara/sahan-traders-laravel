...
type CartRowProps = {
    item: CartItem,
    onChangeQty: (cartItemId: number, qty: number, unitId?: string) => void
}
const CartRow = (props: CartRowProps) => {
    const { item, onChangeQty } = props;
        ...
        <Row className='border-bottom py-2'>
            ...
            <Col md={10}>
                ...
                <Row className='align-items-center'>
                    ...
                    <Col md={3}><QtyInput
                        qty={qty} disableIncrement={false} disableDecrement={false} onChange={(qty, unitId) => onChangeQty(id, qty, unitId)} /></Col>
                    <Col md={3} className=' d-flex justify-content-end'><span>{displayAmount}</span></Col>
                    <Col md={1}><Button variant="danger">
...
const CartStep1Table = (props: CartStep1TableProps) => {
    ...
    return <Container>
        ...
        {cartItems.map((cartItem, index) => (
            <CartRow key={index} item={cartItem} onChangeQty={() => { }} />
        ))}
  </Container>
}
